...
/**
 * Gives always the same instance of a type when called from the same thread. There is one instance for each thread.
 * 
 * So kind of like a SingletonProvider but one for each thread. If your application has only one thread it behaves exactly the same.
 * 
